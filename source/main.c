...
int main(void)
{
  ...
  Rectangle d = {(float)SCREEN_WIDTH/2, (float)SCREEN_HEIGHT/2, TS*2, (float)TS/2};
  ...
  while (!WindowShouldClose()){
    ...
    if(health>0){
      for(int i=0;i<asteroidsLen;i++){
        ...
        if(showDefense && circleCollision(asteroids[i].x, asteroids[i].y, asteroids[i].r, d.x, d.y, d.height)){
          asteroids[i].d = -1;
          score++;
          if(asteroidsLen<100){
            asteroidsLen++;
      ...
      if(showDefense){
        DrawCircle(d.x, d.y, d.height, BLUE);
      }
